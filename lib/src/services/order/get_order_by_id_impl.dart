...
import '../../models/orders/order_product_model.dart';
...
class GetOrderByIdImpl implements GetOrderById {
  ...
  Future<OrderDto> _orderDtoParse(OrderModel order) async {
    ...
    final responses =
        await Future.wait([paymentTypeFuture, userFuture, orderProductFuture]);
    return OrderDto(
      id: order.id,
      date: order.date,
      status: order.status,
  ...
  Future<List<OrderProductDto>> _orderProductParse(OrderModel order) async {
    ...
    for (var i = 0; i < order.orderProducts.length; i++) {
      final OrderProductModel orderProduct = order.orderProducts[i];
      final OrderProductDto productDto = OrderProductDto(
        product: products[i],
